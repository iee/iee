...
public class ExtendedTextViewerUndoManager implements IUndoManager,
	...
	private class DocumentUndoListener implements IDocumentUndoListener {
		...
		/*
		 * @see
		 * org.eclipse.jface.text.IDocumentUndoListener#documentUndoNotification
		 * (DocumentUndoEvent)
		 */
		public void documentUndoNotification(DocumentUndoEvent event) {
			...
			if (((eventType & DocumentUndoEvent.ABOUT_TO_UNDO) != 0)
				...
				fTextViewer.getTextWidget().getDisplay()
	 ...
	 */
	public ExtendedTextViewerUndoManager(int undoLevel) {
	...
	private void openErrorDialog(final String title, final Exception ex) {
		...
		if (Display.getCurrent() != null)
			...
			display.syncExec(new Runnable() {
				public void run() {
					MessageDialog.openError(finalShell, title,
	...
	public void redo() {
		if (isConnected()) {
			try {
				...
				openErrorDialog(
						JFaceTextMessages
								.getString("DefaultUndoManager.error.redoFailed.title"), ex); //$NON-NLS-1$
			}
		}
	}
	...
	public void undo() {
		if (isConnected()) {
			try {
				...
				openErrorDialog(
						JFaceTextMessages
								.getString("DefaultUndoManager.error.undoFailed.title"), ex); //$NON-NLS-1$
			}
		}
	}
	...
	private void connectDocumentUndoManager(IDocument document) {
		...
		if (document != null) {
			...
			fDocumentUndoManager = DocumentUndoManagerRegistry
